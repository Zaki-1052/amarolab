# new-Gi/analysis/phase9_aromatics/p9b_signed_rotation.py
#
# Signed aromatic ring reorientation with pairwise correlations.
# Fixes two issues in p9_aromatics.py:
#   1. arccos always returns positive angles — cross product now gives sign
#   2. cumulative rotation was divided by 360, but arccos maxes at 180
...
ax.set_title('Aromatic ring reorientation (signed)')
...
ax.set_title('Pairwise aromatic rotation correlation')
fig.tight_layout()
...
ax.set_title('Running pairwise aromatic rotation correlation')
ax.set_ylim(-1, 1)
