# new-Gi/analysis/phase4_psilocin/p4_psilocin.py
...
# D155 in literature = PROE resid 76 in 9LL8 build numbering

psilocin_N = u.select_atoms("segid HETA and name N1")
asp76_OD1  = u.select_atoms("segid PROE and resid 76 and name OD1")
asp76_OD2  = u.select_atoms("segid PROE and resid 76 and name OD2")

assert len(psilocin_N) == 1, f"Expected 1 psilocin N1, got {len(psilocin_N)}"
assert len(asp76_OD1) == 1, f"Expected 1 D76 OD1, got {len(asp76_OD1)}"
assert len(asp76_OD2) == 1, f"Expected 1 D76 OD2, got {len(asp76_OD2)}"
print(f"Selections OK: psilocin N1, D76 OD1, D76 OD2 (1 atom each)")

d_OD1 = np.empty(len(u.trajectory))
d_OD2 = np.empty(len(u.trajectory))

for i, ts in enumerate(u.trajectory):
    d_OD1[i] = np.linalg.norm(psilocin_N.positions[0] - asp76_OD1.positions[0])
    d_OD2[i] = np.linalg.norm(psilocin_N.positions[0] - asp76_OD2.positions[0])

salt_bridge = np.minimum(d_OD1, d_OD2)

print(f"\nSalt bridge (N1 — D76 / D155 min distance):")
...
u.trajectory[0]
pocket_dynamic = u.select_atoms("(segid PROE PROF) and (around 5.0 (segid HETA))")
...
ax1.set_title('Psilocin N1 — D76 (D155) salt bridge')
