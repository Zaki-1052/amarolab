...
This file contains multiple functions and classes that can be used to read and write PDB, PQR, and PQRXML formats in versatile ways. It also contains some functions for processing loaded structures, such as center-of-mass calculations, etc.
...
from amarolab.amaro.amaro_scripts.modscience import normalize_vector # makes the longest vector equal to 1
import unittest
...
atomic_weights = {'H':1.00800, 'C':12.0110, 'N':14.00699, 'O':15.9994, 'F':18.9984, 'Na':22.9898, 'Mg':24.305, 'P':30.9738, 'S':32.065, 'Cl':35.453, 'K':39.0983, 'Ca':40.078, 'Fe':55.845, 'Br':79.904, 'I':126.9045, '':0.0, ' ':0.0, 'G':0.0} # all the atomic weights
...
#filename = '/scratch/lvotapka/projects/na_mdbd/md_grid/2hu0_apo_ens/building/2hu0_tami_config0.pdb'
...
def minmax_width (structure, water_only=True, wat_resnames=('TIP','H2O','HOH','WAT')):
  '''returns the minimum and maximum coordinate values in the x,y,z direction'''
  #structure=parser.get_structure("MD_init", pdbfile)
...
class Test_pdb_functions(unittest.TestCase):
  ...
  def test_make_pqr_line(self): # test this function
    testparams=[{'index':3456,'resid':543, 'resname':'ALA', 'name':'CA', 'charge':-1.3, 'radius':2.3},]
    testline=['ATOM   3456  CA  ALA   543       0.000   0.000   0.000 -1.3000 2.3000',]
    for i in range(len(testparams)):
      testresult = make_pqr_line(testparams[i])
      self.assertEqual(len(testresult), 69)
      self.assertEqual(testresult, testline[i])

  # make unit test for the remark in a pqrxml file
  ...
  def test_pdb_parse(self):
    ...
    pdb_line = conventional_parse(test_pdb_line,pqr=False)
    pqr_line = conventional_parse(test_pqr_line,pqr=True)
    pqr_line = conventional_parse(problem_line,pqr=True)
...
if __name__=="__main__":
  ...
  test_struct1_pdb_filename = '/home/lvotapka/scripts/pdb2_test/adi_dry.pdb' # an alanine dipeptide
  test_struct2_pdb_filename = '/home/lvotapka/scripts/pdb2_test/tropc_wet.pdb' # troponin C solvated in H2O
  test_struct3_pdb_filename = '/home/lvotapka/scripts/pdb2_test/cal-2hty-tetra.pdb'
  test_struct1_pqr_filename = '/home/lvotapka/scripts/pdb2_test/tami.pqr'
  test_struct2_pqr_filename = '/home/lvotapka/scripts/pdb2_test/tropc_dry.pqr'
  test_struct3_pqr_filename = '/home/lvotapka/scripts/pdb2_test/cal-2hty-tetra.pqr'
  test_struct1_pqrxml_filename = '/home/lvotapka/scripts/pdb2_test/tami.pqrxml'
