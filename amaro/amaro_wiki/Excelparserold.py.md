...
`
    ...
    Input file format:
     ...
     own set of commands, separated by the 'path' command:
      ...
      - columns (optional) <source column to include, 'none' will omit every column in the pathblock>:<destination of specified column(optional)>
     ...
     all input files should end with the character '/' on its own line
    ...
    def _handlepictures(coltablelist):
      ...
      for eachid in idslist:
        ...
        command2 = "inchi=`babel ghost%s.smi -oinchi 2>/dev/null | awk '{print $1;}'`" % eachid #feed the smile file to babel, output to inchi
        command3 = 'echo "$inchi" > ghost%s.inchi' % eachid #place the value of inchi into another file
        command4 = "URLinchi=`echo \"$inchi\" |  perl -MURI::Escape -lne 'print uri_escape($_)' | sed -e's/%2F/\//g'`" #create the URLinchi variable which will be fed to the url
        command5 = 'curl -s -o %s/%s.png "http://cactus.nci.nih.gov/chemical/structure/${URLinchi}/image?format=png&height=%i&width=%i&showstereo=0"' % (picturepath,eachid,picturewidth,pictureheight) #now access the online database for a picture according to these specifications
        command6 = 'rm ghost%s.smi ; rm ghost%s.inchi' % (eachid,eachid) #delete the .inchi and .smi files, now unnecessary
        allcommand = '\n'.join((command1,command2,command3,command4,command5,command6)) #splice the commands together
        #print allcommand
        if i%100 == 0: print 'obtaining picture %i out of %i' % (i,ifinal) #keep the user updated every 100 pictures
        os.system(allcommand) #feed the spliced commands into the os
    ...
    def _createspreadsheet(coltablelist,picloc,idslist,destpath):
      ...
      if picloc <> '':
        ...
        for eachid in idslist: #for each id
          ...
          #print 'picturepath: ', picturepath
          sheet.insert_bitmap(img2bmp(indpngpath),i,picloc) #insert the picture into the spreadsheet
