...
`
    ...
    def make_atom_occ (atom, grid, outcomes, num):
      ...
      if atom.radius <= 0.0: return # skip atoms with no radius
      lastpoint = [0,0,0]
      ...
      for i in range(closegridx - proximity, closegridx + proximity + 1):
        ...
        for j in range(closegridy - proximity, closegridy + proximity + 1):
          ...
          for k in range(closegridz - proximity, closegridz + proximity + 1):
            ...
            if grid[i*nz*ny+j*nz+k] == 1: continue
            ...
            if np.dot(dist,dist) < radius*radius:
              #print "close:", i, j, k
              grid[i*nz*ny+j*nz+k] = 1
              lastpoint = [i,j,k]
              
      #print lastpoint
      #print "num:", num, "type:", type(num), "len(outcomes):", len(outcomes)
      outcomes[num,0] = lastpoint[0]; outcomes[num,1] = lastpoint[1]; outcomes[num,2] = lastpoint[2]
      #print "inside outcomes[num,:]", outcomes[num,:]
      return 
    ...
    parser.add_argument('-t', '-threads', dest="threads", type=int, default=1, help="number of threads to use when running a multiprocess")
    ...
    mp_grid = mp.Array(c.c_int, nx*ny*nz) # make a large 1d array
    grid = np.zeros((nx, ny, nz), dtype=np.int8)
    #grid_1d = np.frombuffer(mp_grid.get_obj(), dtype = c.c_int)
    #grid = grid_1d.reshape((nx,ny,nz))
    ...
    #p = Process(target=make_atom_occ, args=()) # create a parallel object
    #outcomes = p.map(make_atom_occ, [pqr.atoms, grid]) # perform the parallel calculation
    #for i in range(len(outcomes), 0, -1):
    ...
    mp_outcomes = mp.Array(c.c_int, [0]*len(pqr.atoms)*3)
    outcomes_1d = np.frombuffer(mp_outcomes.get_obj(), dtype = c.c_int)
    outcomes = outcomes_1d.reshape((len(pqr.atoms), 3))
    
    counter = 0 #mp.Value('i', 0)
    for atom in pqr.atoms:
      p = mp.Process(target = make_atom_occ, args = (atom, mp_grid, outcomes, counter))
      jobs.append(p)
      p.start()
      p.join()
      counter += 1
    
    for i in range(len(pqr.atoms)):
      for j in range(3):
        outcomes[i,j] = mp_outcomes[i*3 + j]
      #print "outcomes[%d,:]:" % i, outcomes[i,:]
    
    for i in range(len(mp_outcomes)/3, 0, -1):
      #outcome = make_atom_occ(atom) # the way it was before threading
      outcome = outcomes[i-1,:]
      #print "i:", i, "outcome[i]:", outcome
      if np.count_nonzero(outcome) > 0: lastpoint = outcome
    
    
    for i in range(nx):
      for j in range(ny):
        for k in range(nz):
          #if mp_grid[i*nx*ny+j*nx+k] != 0: print mp_grid[i*nx*ny+j*nx+k]
          grid[i,j,k] = mp_grid[i*nz*ny+j*nz+k]
    
    #print "len(pqr.atoms):", len(pqr.atoms)
    
    #for atom in pqr.atoms:
    #  outcome = make_atom_occ(atom) # the way it was before threading
    #  if outcome: lastpoint = outcome
