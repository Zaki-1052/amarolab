...
`
    ...
    ################### the IF command ############################
          if split_command[0] == "if":
            ...
            if includingblock: # if we are including the superblock
    ...
    ################### ELSEIF command #######################################
            ...
            if not includingblock and non_including_depth == 1 : # if the last statement was false, and we aren't in a non-included superblock
              openblock.pop()
              openblock.append(blocks[command_count])
              evaluation = self._statement_eval(split_command, var_pat)
              if evaluation: # then we can include this block
    ...
    ################### ENDIF command #######################################
            ...
            if not includingblock: # then we need to see whether ending this block will
    ...
    ################### COMMENT command #######################################
          elif split_command[0] == "comment": # close the IF block
            openblock.append(blocks[command_count])
            non_including_depth += 1
            includingblock = False
    
    ################### ENDCOMMENT command #######################################
          elif split_command[0] == "endcomment": # close the IF block
            openblock.pop()
            if non_including_depth > 0: # if we didn't include this block
              non_including_depth -= 1 # reduce the depth
            if non_including_depth == 0: # then this was the highest superblock that wasn't included
    ...
    class Test_adv_template(unittest.TestCase):
      ...
      def test_File_template(self):
        ...
        template = "{% if $NAME=='Johnny' %}Hi Johnny{% else %}Hi someone else{% endif %}"
        params = {'NAME':'Johnny', 'AGE':'thirty'}
