...
# Two contiguous blocks (don't span the ICL3 gap at PROB 185 / PROC 186)
block1 = seq[seq['resid'] <= 185].to_dict('records')   # PROA + PROB
block2 = seq[seq['resid'] >= 186].to_dict('records')   # PROC
...
# Deduplicate: group by the central residue (Y or Y/F), keep shortest motif
...
print("=" * 65)
print(f"Scanned 266 receptor residues in 2 contiguous blocks")
print(f"  Block 1: PROA+PROB resids 1-185 (lit 79-263)")
print(f"  Block 2: PROC resids 186-266 (lit 313-393)")
...
for m in crac_dedup:
    ...
    # Build the sequence snippet between anchors
...
print(f"\nSpearman correlation (all 266 residues): rho={r_all:.3f}, p={p_all:.2e}")
