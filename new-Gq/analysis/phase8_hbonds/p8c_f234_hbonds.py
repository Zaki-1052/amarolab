# new-Gq/analysis/phase8_hbonds/p8c_f234_hbonds.py
#
# H-bond analysis for the F234 neighborhood — Gq version.
...
u.trajectory[0]
f234_sel = "segid PROB and resid 156 and not name H*"
f234_atoms = u.select_atoms(f234_sel)
print(f"F234 heavy atoms: {len(f234_atoms)}")

neighborhood = u.select_atoms(
    f"(segid PROA PROB PROC HETA) and around {NEIGHBORHOOD_CUTOFF} ({f234_sel})")
...
if top_bonds:
    ...
    ax.set_title(f'H-bond occupancy — F234 neighborhood ({NEIGHBORHOOD_CUTOFF} Å, Gq)')
    ax.invert_yaxis()
...
if map_bonds:
    ...
    ax.set_title('F234 neighborhood H-bonds: formation and breaking (Gq)')
